...
const client_secret = process.env.TWITTER_CLIENT_SECRET || null;
...
if (!client_id) {
  console.error("Missing TWITTER_CLIENT_ID. Set env or .env.local file.");
  process.exit(1);
}
...
http.createServer(async (req, res) => {
  ...
  // Also update .env.local with the refresh token
  try {
    const envPath = join(__dirname, ".env.local");
    let envContent = readFileSync(envPath, "utf8");
    
    // Update or add TWITTER_REFRESH_TOKEN
    if (envContent.includes("TWITTER_REFRESH_TOKEN=")) {
      envContent = envContent.replace(
        /TWITTER_REFRESH_TOKEN=.*/,
        `TWITTER_REFRESH_TOKEN=${json.refresh_token}`
      );
    } else {
      envContent += `\nTWITTER_REFRESH_TOKEN=${json.refresh_token}`;
    }
    ...
    console.log("Updated .env.local with refresh token");
  } catch (error) {
    console.log("Could not update .env.local:", error.message);
  }
  
  res.end("Tokens received and saved. You can close this tab.");
  process.exit(0);
